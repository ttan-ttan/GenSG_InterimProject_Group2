...
def _token() -> str:
    ...
    if not token:
        raise RuntimeError(
            "ONEMAP_TOKEN is not set in the environment (.env file).")
    return token
...
def parse_locality(locality: str) -> List[Tuple[str, str]]:
    ...
    for segment in locality.split("/"):
        ...
        if match:
            road_text, paren = match.group(
                "road"), match.group("paren").strip()
        else:
        ...
        if paren and BLK_RE.match(paren):
            ...
            blocks = [b.strip() for b in BLK_RE.sub(
                "", paren).split(",") if b.strip()]
            queries.extend(
    ...
    for lat, lon in sample_points(geometry, max_points):
        for building in reverse_geocode(lat, lon):
            ...
            address = " ".join(x for x in (
                block, road, name) if x and x != "NULL")
...
def main() -> None:
    ...
    if args.geojson and args.save_raw:
        parser.error(
            "--save-raw applies to the live download, so it cannot be used with --geojson.")
    ...
    for feature in features:
        ...
        if not found and not args.no_fallback:
            fallback = resolve_geometry(
                feature.get("geometry", {}), locality_roads(
                    locality), args.geom_points
            )
